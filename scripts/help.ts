...
const WORKFLOWS: Workflow[] = [
  ...
  },
  {
    ...
    emoji:       "📦",
    description: "Alternativa CLI al flujo manual 🔍 para cuando quieres llenar el banco con N refs por imagen sin ir card a card. Itera el banco, salta los SHAs que YA tienen target (default 5) o que SON refs ellos mismos, y descarga lo que falte. Persiste tras cada SHA → resistente a crashes (re-ejecutar reanuda). Default Google Lens (calidad alta, gasta SerpAPI ~1 búsqueda/SHA); --provider stock para usar Pexels+Pixabay free si no quieres tocar cuota.",
    steps: [
      { cmd: "npm run images:bulk-save-refs -- --dry-run",                 comment: "ver cuántas SHAs procesaría + coste estimado SerpAPI" },
      { cmd: "npm run images:bulk-save-refs -- --max-shas 5",              comment: "smoke test con 5 SHAs antes de tirarlo del todo" },
      { cmd: "npm run images:bulk-save-refs -- --max-shas 100 --yes-burn-serpapi", comment: "tanda completa quemando el free tier SerpAPI mensual (100 búsquedas)" },
      { cmd: "npm run images:classify-modal",                              comment: "tras el bulk, clasifica las refs huérfanas (PENDIENTE → clasificada)" },
      { cmd: "# (al mes siguiente) repetir desde paso 3",                  comment: "el registry tiene contador per-SHA — automaticamente reanuda con los que aún no llegan a 5" },
    ],
  },
  {
    ...
    scripts: [
      ...
      },
      {
        ...
        args: [
          ...
          { name: "--provider <id>",     type: "google | stock", default: "google", description: "google=SerpAPI Lens (reverse image visual, calidad alta) · stock=Pexels+Pixabay (free, keyword search, calidad menor)." },
          { name: "--max-shas <N>",      type: "number",         default: "0",      description: "Limita a las primeras N SHAs (0=todas, útil para smoke test y para respetar cuota free SerpAPI 100/mes)." },
          ...
          { name: "--yes-burn-serpapi",  type: "bool",           default: "off",    description: "Skipea el prompt de confirmación cuando vas a consumir > 50 búsquedas SerpAPI en este run." },
        ],
        examples: [
          "npm run images:bulk-save-refs -- --dry-run                       # ver plan + coste SerpAPI",
          "npm run images:bulk-save-refs -- --max-shas 50                   # 1 run completo del free tier SerpAPI",
          "npm run images:bulk-save-refs -- --max-shas 100 --yes-burn-serpapi  # quema TODA la cuota free mensual",
          "npm run images:bulk-save-refs -- --provider stock                # alternativa free pero calidad menor",
